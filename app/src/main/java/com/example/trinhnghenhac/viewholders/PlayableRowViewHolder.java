...
import com.example.trinhnghenhac.databinding.ListitemPlayableRowBinding;
...
public class PlayableRowViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    private ListitemPlayableRowBinding b;

    public PlayableRowViewHolder(@NonNull View itemView, int viewType, boolean bindEvent, boolean showType, boolean showPlatform) {
        super(itemView);
        ...
        b = ListitemPlayableRowBinding.bind(itemView);
    }

    public void setPlayable(Playable playable) {
        mPlayable = playable;
        if (playable instanceof Playlist || playable instanceof Artist)
            b.listitemPlayableRowAddSaved.setVisibility(View.GONE);
        if (!playable.isPremium()) b.listitemPlayableRowPro.setVisibility(View.GONE);
        b.listitemPlayableRowTitle.setText(playable.getTitle());
        b.listitemPlayableRowText.setText(playable.getText());
        Glide.with(mContext).load(playable.getImage()).into(b.listitemPlayableRowImage);
        if (!mShowType)
            b.listitemPlayableRowType.setVisibility(View.GONE);
        else b.listitemPlayableRowType.setImageResource(Playable.getIcon(playable));
        if (!mShowPlatform)
            b.listitemPlayableRowPlatform.setVisibility(View.GONE);
        else b.listitemPlayableRowPlatform.setImageResource(MusicPlatformApi.toResId(playable.getPlatform()));

        if (mBindEvent && mViewType == PlayableItemType.ITEM_TYPE_PLAYABLE) {
            itemView.setOnClickListener(this);
            b.listitemPlayableRowAddSaved.setOnClickListener(this);
            b.listitemPlayableRowStar.setOnClickListener(this);
        }
    }
    ...
    public void setStarredFrom(List<SavedStarredItem> items) {
        for (SavedStarredItem item : items) {
            ...
            if (mPlayable.getId().equals(item.getItemId()) &&
                ...
                mItem = item;
                b.listitemPlayableRowStar.setIconResource(R.drawable.round_star_24);
                break;
            }
        }
    }
    ...
    public void onClick(View view) {
        if (view == b.listitemPlayableRowAddSaved) {
        ...
        } else if (view == b.listitemPlayableRowStar) {
            if (mItem != null) {
                FirebaseUtils.deleteSaved(mItem).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void unused) {
                        b.listitemPlayableRowStar.setIconResource(R.drawable.round_star_border_24);
                        mItem = null;
                    }
